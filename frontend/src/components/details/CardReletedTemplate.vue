<template>
  <v-carousel-item v-for="n in 3" :key="n">
    <div class="d-flex justify-space-evenly flex-wrap">
      <div class="card pa-2 rounded bg-white" v-for="(event, index) in eventStore.reletedEvent" :key="index">
...
onMounted(() => {
  ...
  eventStore.getDataCategoryAxios(props.eventInfor.category_id, props.eventInfor.id);
